...
public class ARBinding
{
    ...
    public static Texture2D startCameraCapture( bool useFrontCameraIfAvailable, ARQuality quality )
    {
    	...
    	else if( iPhone.generation == iPhoneGeneration.iPhone4 || iPhone.generation == iPhoneGeneration.iPodTouch4Gen )
    	{
    	...
    	else if( iPhone.generation == iPhoneGeneration.Unknown && ( Screen.width == 1024 || Screen.height == 1024 ) ) // ghetto iPad 2 detection
    	{
    		if( useFrontCameraIfAvailable )
			{
	    		width = useLowQuality ? 192 : 480;
